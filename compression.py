...
class VBEPostings:
    ...
    def vb_encode_number(number):
        ...
        bytes_of_num = []
        while True:
            bytes_of_num.insert(0, number%128)
            if number < 128:
                break
            number //= 128
        bytes_of_num[-1] += 128
        return bytearray(bytes_of_num)
    ...
    def encode(postings_list):
        ...
        for i in range(len(postings_list)-1):
            gap_list.append(postings_list[i+1] - postings_list[i])
        bytes_posting_list = VBEPostings.vb_encode(gap_list)
        return bytes_posting_list
    ...
    def decode(encoded_postings_list):
        ...
        if gap_list == []:
            return []
        original_list = [gap_list[0]]
        for i in range(len(gap_list)-1):
            original_list.append(original_list[i]+gap_list[i+1])
